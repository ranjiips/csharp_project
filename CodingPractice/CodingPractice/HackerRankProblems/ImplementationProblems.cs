...
namespace CodingPractice.HackerRankProblems
{
    public class ImplementationProblems
    {
        ...
        public void AngryProfessor()
        {
             ...
             * Frustrated with their lack of discipline, the professor decides to cancel class if fewer 
            ...
            StaticClassExamples.ColorfulWriteLine("\n*********** Angry Professor ***********\n", ConsoleColor.Green);
            int k= 7;
            List<int> a = new List<int> { 26, 94, -95, 34, 67, -97, 17, 52, 1, 86 };
            int studentPresents = a.Where(e => e <= 0).Count();

            if (studentPresents >= k)
                Console.WriteLine($"Number of student present are {studentPresents}, expectd is {k}, Class will continue - NO");
            else
                Console.WriteLine($"Number of student present are {studentPresents}, expectd is {k}, Class will not continue - YES");
        }
    }
}
